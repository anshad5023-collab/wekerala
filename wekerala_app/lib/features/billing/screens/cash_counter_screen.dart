...
class _CashCounterScreenState extends ConsumerState<CashCounterScreen> {
  ...
  double get _handCount => _denoms.fold(0.0, (acc, d) {
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              final billCount = (summary['billCount'] ?? 0.0).toInt();
              final report = '''
📊 DAY-END REPORT — $dateStr

💰 Total Sales:     ₹${total.toStringAsFixed(0)}
🧾 Bills:           $billCount

💵 Cash Collected:  ₹${systemCash.toStringAsFixed(0)}
📱 UPI Collected:   ₹${upi.toStringAsFixed(0)}
📒 Udhar Given:     ₹${udhar.toStringAsFixed(0)}

🖐 Hand Count:      ₹${handCount.toStringAsFixed(0)}
${variance >= 0 ? '✅' : '⚠️'} Variance:         ${variance >= 0 ? '+' : ''}₹${variance.toStringAsFixed(0)}

Generated by Oratas''';
              Share.share(report, subject: 'Day-End Report $dateStr');
            },
          ),
        ],
      ),
          ...
          Container(
            ...
            child: SizedBox(
              ...
              child: OutlinedButton(
                ...
                onPressed: () async {
                  final hasEntries = _controllers.values.any((c) => c.text.isNotEmpty);
                  if (hasEntries) {
                    final confirmed = await showDialog<bool>(
                      context: context,
                      builder: (_) => AlertDialog(
                        title: const Text('Reset Count?'),
                        content: const Text('Clear all denomination entries and start over?'),
                        actions: [
